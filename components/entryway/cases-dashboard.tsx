...
const STAGE_FILTER_LABELS: { value: string; label: string }[] = [
    ...
    <div className="flex h-full flex-col overflow-y-auto">
      <div className="mx-auto w-full max-w-5xl space-y-6 px-6 py-8">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              Manage your active matters and upload new documents.
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input
              ...
              className="glass w-full rounded-lg border border-border/40 bg-transparent py-1.5 pl-8 pr-3 text-sm placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-primary"
            />
          </div>
          <div className="flex flex-wrap gap-1.5">
            {STAGE_FILTER_LABELS.map(({ value, label }) => (
              <button
                key={value}
                type="button"
                onClick={() => setStageFilter((prev) => prev === value ? null : value)}
                className={cn(
            ...
            {stageFilter && (
              <button
                ...
                className="rounded-full border border-border/40 px-2.5 py-0.5 text-[11px] text-muted-foreground/60 transition-colors hover:text-muted-foreground"
              >
                Clear
              </button>
            )}
        ...
        {isLoading ? (
          ...
          <div className="glass flex flex-col items-center justify-center gap-3 rounded-xl py-12 text-center">
            {cases.length === 0 ? (
              <>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-auto flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    {c.next_deadline && (
                      <span className={cn(
                        new Date(c.next_deadline).getTime() - Date.now() < 7 * 86_400_000
                          ? "text-amber-400"
                          : "",
                      )}>
                        Due {relativeDate(c.next_deadline)}
                      </span>
                    )}
                  </div>
                </Link>
